...
if not opt then
    ...
    cmd:option('-maxEpoch', 100, 'max number of epoch')
    cmd:option('-manualSeed', 1234, 'manual seed for Torch random')
    cmd:text()
...
function test()
    model:evaluate()
    local f = torch.DiskFile(opt.result_path .. '/result-' .. best_val .. '.txt', 'w')
...
worker['options'] = opt
torch.save(opt.worker_path .. '/worker-' .. best_val .. '.t7', worker)
